...
- 在设计页填写“设计名称”（选填，最多 80 个字符），保存或提交后会显示在“我的设计”、详情页和排行榜中。旧记录或未命名设计继续显示设计编号。
- 在“我的设计”点击“预览”，可查看该条保存序列的二级结构。预览使用 ViennaRNA 计算，不依赖管理员上传三维结构，也不会修改设计或评分。
- 点击“从此处开始设计”，会载入所选版本的名称和序列。保存时新建草稿，保留原提交、原评分及其他未完成草稿。新草稿可以改名后再次提交。
- 从历史版本进入后，设计页和参考页同时显示初始 HEPHA 结构与上一版设计；上一版沿用原名称、序列、结构和评分。保存后的草稿链接保留所选参考版本。
- 草稿行提供“继续编辑草稿”，用于回到指定的未完成版本。
- 排行榜仍按每位玩家已公布的最佳成绩排名，显示对应设计名称。
- 后端启动时自动给现有 SQLite 数据库补充可为空的 name 字段，保留已有记录。
